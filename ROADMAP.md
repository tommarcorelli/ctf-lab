...
- [x] **i18n FR/EN** — internationalisation FR/EN avec bascule à chaud (bouton **🌐**, `LANG` persisté,
      deep-link `index.html#en`). `js/i18n.js` (dictionnaire, `t()`/`applyI18n()`/`setLang()`) traduit
      tout le **chrome de l'UI** (en-tête, info-bulles, sidebar dont difficultés & « verrouillé »,
      les 5 modales, bannière) via `data-i18n`/`data-i18n-html`/`data-i18n-title`. Côté moteur, un
      helper `bilang(fr, en)` (le moteur lit `LANG`) rend bilingues l'**aide** (`help`), les
      **briefings des 12 machines** (`BRIEFING_EN`), le message de **cible active**, l'accueil de
      connexion et les **toasts de progression** (recon/accès/privesc/flags). *Reste FR (choix
      assumé, documenté)* : les **indices/lore** détaillés des machines et une partie des sorties de
      commandes — du contenu, pas un manque d'infra ; `bilang`/`t` permettent de les traduire au fil
      de l'eau. Testé dans `tests/run.js` (`bilang`, briefing EN de NIMBUS, repli FR) et vérifié au
      rendu (UI + sidebar + bannière + gameplay en anglais).
